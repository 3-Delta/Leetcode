...
public class MaxHeap<T> where T : IPriority
{
    ...
    private void Down(int index)
    {
        while (HasLeftChild(index))
        {
            ...
            // 左右节点优先级比较，获取优先级较大者
            if (HasRightChild(index) && list[subIndex].Priority() < list[subIndex + 1].Priority())
            {
                ++subIndex;
            }
    ...
    {
        while (index > 0 && list[index].Priority() > list[(index - 1) / 2].Priority())
        {
    ...
    {
        if (!list.IsIndexValid(leftIndex) || !list.IsIndexValid(rightIndex)) { return; }
        T left = list[leftIndex];
        T right = list[rightIndex];
        Tool.Swap<T>(ref left, ref right);
        list[leftIndex] = left;
